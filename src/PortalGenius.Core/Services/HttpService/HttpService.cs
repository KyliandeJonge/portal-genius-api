...
public class HttpService : IHttpService
{
    ...
    public async Task<T> GetAsync<T>(string path)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
                ...
                _logger.LogWarning("[HTTP GET {statusCode}] Something went wrong while connecting with: ({apiUrl}).",
            ...
            var response = await _httpClient
                ...
                .ContinueWith(async searchTask =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            _logger.LogWarning(
                                "[HTTP GET {statusCode}] Something went wrong while connecting with: ({apiUrl}).",
                                response.StatusCode, apiUrl);
                        }
                    }
                    catch (HttpRequestException ex)
    ...
    public async Task<T> PostAsync<T>(string path, StringContent stringContent)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
                ...
                _logger.LogWarning("[HTTP POST {statusCode}] Something went wrong while connecting with: ({apiUrl}).",
                    response.StatusCode, apiUrl);
        }
        catch (HttpRequestException ex)
        {
    ...
    {
        return $"{_httpClient.BaseAddress.ToString().Trim('/')}/{path}";
    }
}
