...
public class UpdateItemsService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            if (TimeSpan.TryParse(_configuration["DataRefreshInterval"], out var interval))
            {
                await Task.Delay(interval, cancellationToken);
            }
            else
            {
                _logger.LogError("Invalid configuration value '{interval}' for property 'DataRefreshInterval'",
                    _configuration["DataRefreshInterval"]);
                break;
            }
        }
    }
